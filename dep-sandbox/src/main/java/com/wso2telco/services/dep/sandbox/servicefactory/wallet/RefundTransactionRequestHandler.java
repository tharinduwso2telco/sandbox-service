...
public class RefundTransactionRequestHandler extends AbstractRequestHandler<RefundRequestWrapperDTO>{
	...
	@Override
	protected void init(RefundRequestWrapperDTO extendedRequestDTO)
			throws Exception {
	...
	@Override
	protected boolean validate(RefundRequestWrapperDTO wrapperDTO)
			throws Exception {
		...
		PaymentAmount paymentAmount = request.getPaymentAmount();
				...
				ChargingMetaData metaData = paymentAmount.getChargingMetaData();				
					...
					try {
						...
						validationRulesList.add(new ValidationRule(ValidationRule.VALIDATION_TYPE_MANDATORY, "originalReferenceCode", originalReferenceCode));
						validationRulesList.add(new ValidationRule(ValidationRule.VALIDATION_TYPE_MANDATORY, "originalServerReferenceCode", originalServerReferenceCode));
	...
	@Override
	protected Returnable process(RefundRequestWrapperDTO extendedRequestDTO)
			throws Exception {
		...
		if(!isValidCurrency){
		    ...
		    responseWrapper
			    .setRequestError(constructRequestError(SERVICEEXCEPTION,
				    ServiceError.INVALID_INPUT_VALUE,
				    "currency code not as per ISO 4217"));
		    responseWrapper.setHttpStatus(Status.BAD_REQUEST);
		    return responseWrapper;
		...
		responseBean.setOriginalServerReferenceCode(originalServerReferenceCode);
			...
			String accountStatus = walletDAO.getAttributeValue(endUserId, serviceCallRefund,AttributeName.transactionStatus.toString());
			...
			/*else {
				...
				responseWrapper
						.setRequestError(constructRequestError(POLICYEXCEPTION, PolicyError.NO_VALID_SERVICES_AVAILABLE, "Security Isuue"));
				return responseWrapper;
			}*/
	...
	public void saveTransaction(RefundTransactionResponseBean responseBean , String endUserId, boolean metaData) throws Exception{

...
public class AssignAccountInfoConfigRequestHandler extends AbstractRequestHandler<AssignAccountInfoConfigRequestWrapper>{
	
	AssignAccountInfoConfigRequestWrapper requestWrapper;
	AssignAccountInfoConfigResponseWrapper responseWrapper;
	WalletDAO walletDAO;
	...
	@Override
	protected boolean validate(AssignAccountInfoConfigRequestWrapper wrapperDTO)
			throws Exception {
		...
		AssignAccountInfoConfigRequestBean requestBean = wrapperDTO.getRequestBean();
			...
			try {				
				...
				validationRulesList.add(new ValidationRule(ValidationRule.VALIDATION_TYPE_MANDATORY_CURRENCY,"accountCurrency",accountCurrency));
				validationRulesList.add(new ValidationRule(ValidationRule.VALIDATION_TYPE_MANDATORY,"accountStatus",accountStatus));
	...
	protected Returnable process(
			AssignAccountInfoConfigRequestWrapper extendedRequestDTO)
			throws Exception {
		...
		try{
			...
			String activeStatus = AccountStatus.ACTIVE.toString();
			String suspendedStatus = AccountStatus.SUSPENDED.toString();
			String terminatedStatus = AccountStatus.TERMINATED.toString();
			...
			if(!(status.equals(activeStatus) || status.equals(suspendedStatus) || status.equals(terminatedStatus))){
			...
			APITypes apiType = dao.getAPIType(RequestType.WALLET.toString()
					.toLowerCase());
			APIServiceCalls serviceType = dao.getServiceCall(apiType.getId(),
					ServiceName.BalanceLookup.toString());
			...
			List<AttributeDistribution> availableDistribution = dao
				...
				valuesInRequset.put("status", status);
				...
				for (Map.Entry<AttributeDistribution, String> eachValuedDistribution : attributeValueDistribution
					.entrySet()) {
				    valueObj = walletDAO.getAttributeValue(eachValuedDistribution.getKey(),ownerId);
				...
				responseWrapper
					.setStatus("Successfully Updated attribute for accountInfo!!");
	...
	@Override
	protected void init(AssignAccountInfoConfigRequestWrapper extendedRequestDTO)
			throws Exception {

...
public class RemoveProvisionedServices extends
		AbstractRequestHandler<RemoveProvisionedRequestWrapperDTO> {
	...
	protected boolean validate(RemoveProvisionedRequestWrapperDTO wrapperDTO)
		...
		RemoveProvisionRequestBean requestBean = wrapperDTO
		...
		if (requestBean != null && request != null) {
			...
			if (callRef != null) {
				...
				try {
					...
					if ((mcc != null && mcc.trim().length() > 0)
						...
						validationRules.add(new ValidationRule(
								ValidationRule.VALIDATION_TYPE_OPTIONAL, "mcc",
								mcc));
						validationRules.add(new ValidationRule(
								ValidationRule.VALIDATION_TYPE_OPTIONAL, "mnc",
								mnc));
					}
				...
				responseWrapper.setRequestError(constructRequestError(
						SERVICEEXCEPTION,
						ServiceError.INVALID_INPUT_VALUE.getCode(),
						ServiceError.INVALID_INPUT_VALUE.getMessage(),
	...
	protected Returnable process(
			RemoveProvisionedRequestWrapperDTO extendedRequestDTO)
			throws Exception {
		...
		RemoveProvisionRequestBean requestBean = extendedRequestDTO
				.getRemoveProvisionRequestBean();
		ServiceRemoveRequest request = requestBean.getServiceRemoveRequest();
		...
		String clientCorrelator = CommonUtil.getNullOrTrimmedValue(request
				.getClientCorrelator());
		String clientReferenceCode = CommonUtil.getNullOrTrimmedValue(request
				.getClientReferenceCode());
		String notifyURL = CommonUtil.getNullOrTrimmedValue(request
				.getCallbackReference().getNotifyURL());
		String callbackData = CommonUtil.getNullOrTrimmedValue(request
		...
		try {
			...
			ManageNumber mappedNumber = dao.getMSISDN(phoneNumber, null, mcc,
			...
			ProvisioningUtil.saveProvisioningRequestDataLog(
					ProvisionRequestTypes.DELETE_PROVISION_SERVICE.toString(),
					extendedRequestDTO.getMsisdn(), user, clientCorrelator,
					clientReferenceCode, notifyURL, callbackData, new Date());
			...
			checkServiceValidityForMSISDN(phoneNumber, user.getUserName(),
					availableService);
			...
			ProvisionedServices provisionedCheckList = provisioningDao
					.getAlreadyProvisioned(phoneNumber, user.getUserName(),
							serviceCode);

			if (provisionedCheckList != null) {
				// update db for removal of provisioned service
				ProvisionedServices deletedServiceList = removeProvisionedService(
						provisionedCheckList, clientCorrelator,
				...
				// duplication against client correlator
				boolean isDuplicate = checkRequestDuplication(phoneNumber,
						user.getUserName(), serviceCode, clientCorrelator);
				// non matching client correlator
				if (!isDuplicate) {
					buildJsonResponseBody(
							serviceCode,
							clientCorrelator,
							clientReferenceCode,
							callbackData,
							notifyURL,
							statusMap
									.get(ProvisioningStatusCodes.PRV_DELETE_NOT_ACTIVE)
									.getStatus());
					return responseWrapper;
				}
			}
			...
			LOG.error(
					"###PROVISION### Error in processing provision service request. ",
					ex);
			if (responseWrapper.getRequestError() != null) {
				return responseWrapper;
			}
			buildJsonResponseBody(serviceCode, clientCorrelator,
					clientReferenceCode, callbackData, notifyURL, statusMap
							.get(ProvisioningStatusCodes.PRV_DELETE_FAILED)
							.getStatus());
			return responseWrapper;
		}
		return responseWrapper;
	 ...
	 */
	@Override
	protected void init(RemoveProvisionedRequestWrapperDTO extendedRequestDTO)
			throws Exception {
	...
	private boolean checkRequestDuplication(String msisdn, String userName,
			String serviceCode, String clientCorrelator) throws Exception {
		ProvisionedServices service = provisioningDao.checkClientCorrelator(
				msisdn, userName, serviceCode, clientCorrelator);
	...
	private ProvisionedServices removeProvisionedService(
			ProvisionedServices provisionedCheckList, String clientCorrelator,
			String clientReferenceCode, String notifyURL, String callbackData,
		...
		provisionedCheckList.setStatus(statusMap
				.get(ProvisioningUtil.DEFAULT_REMOVE_STATUS));
	...
	private void checkIfErrorMessageSet(String msisdn, String userName,
			String serviceCode) throws Exception {

		ProvisionResponseMessage errorResponse = provisioningDao
				.getErrorResponse(msisdn, userName, serviceCode);
		if (errorResponse != null) {
			responseWrapper.setRequestError(constructRequestError(errorResponse
					.getResponseMessageCatergory().getId(), errorResponse
					.getResponseCode(), errorResponse.getResponseMessage(),
					msisdn));
	...
	private void checkServiceValidityForMSISDN(String msisdn, String userName,
			ProvisionAllService availableService) throws Exception {
		...
		if (serviceCheckList == null) {
			responseWrapper.setRequestError(constructRequestError(
					...
					"Number is not Registered for the Service"));
			responseWrapper.setHttpStatus(Response.Status.BAD_REQUEST);
	...
	private Map<ProvisioningStatusCodes, Status> getAllTransactionStatus()
			throws Exception {

		Map<ProvisioningStatusCodes, Status> statusMap = new HashMap<ProvisioningStatusCodes, Status>();
		List<Status> status = provisioningDao.getTransactionStatus();
		for (Status states : status) {
			statusMap.put(ProvisioningStatusCodes.valueOf(states.getCode()),
					states);
		}
		return statusMap;
	}

...
class HibernateCommonDAO extends AbstractDAO implements GenaricDAO {
	...
	public void saveAttributeValue(List<AttributeValues> valueObj)
			throws Exception {
	...
	public List<AttributeValues> getAttributeListValue(
		...
		try {
			value = (List<AttributeValues>) sess
					...
					.setParameterList("distributionObj", distributionObj)
					.list();
		} catch (Exception e) {
			LOG.error("getAttributeValue", e);
			throw e;
	...
	@Override
	public AttributeValues getAttributeValue(
			AttributeDistribution distributionObj, int ownerdid)
			throws Exception {
		Session sess = getSession();
		AttributeValues value = null;
		try {
			value = (AttributeValues) sess
					...
					.setParameter(0, distributionObj).setParameter(1, ownerdid)
					.uniqueResult();
		} catch (Exception e) {
			LOG.error("getAttributeValue", e);
			throw e;
	...
	@Override
	public ManageNumber getMSISDN(String msisdn, String imsi, String mcc,
			String mnc) throws Exception {
		Session session = getSession();
		...
		try {
			...
			for (Entry<String, String> parameterEntry : entrySet) {
				if (parameterEntry.getKey().equals("mcc")
						|| parameterEntry.getKey().equals("mnc")) {
					query.setParameter(parameterEntry.getKey(),
							Integer.parseInt(parameterEntry.getValue()));
				} else {
					query.setParameter(parameterEntry.getKey(),
